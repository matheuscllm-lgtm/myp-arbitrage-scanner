"""Entrega canônica do scan MYP → ONE PIECE: XLSX → markdown.

Espelho do `myp_dbz_summary.py`/`myp_summary.py` (mesmo contrato de entrega do
repo: rodar a ferramenta e colar o markdown VERBATIM no chat — nunca remontar
tabela à mão, nunca dropar o link [TCG]). Lê o XLSX gerado pelo
`myp_op_scanner.py`.
...
Uso:
    python myp_op_summary.py results/myp_op_<ts>.xlsx \
        -o results/op-2026-08-09.md [--run-id N]
"""
...
# Reusa o link de busca do scanner OP (linhas sem produto tcgcsv casado).
# Import tolerante (mesmo padrão do myp_summary.py): sem o módulo, o link de
# busca some mas a tabela segue.
try:
    from myp_op_scanner import tcg_op_search_url
except Exception:  # pragma: no cover - fallback defensivo
    def tcg_op_search_url(name, code=None):
        return None
...
def carta_label(name, code) -> str:
    """Coluna `Carta` = nome + código numa string só ('Monkey.D.Luffy
    OP01-003'). Não duplica quando o código já está no nome (padrão da
    frota)."""
    n = str(name or "").strip()
    c = str(code or "").strip()
...
def delivery_links(myp_url, tcg_url, name=None, code=None) -> str:
    ...
    tcg = tcg_url or tcg_op_search_url(str(name or ""), str(code or "") or None)
    if tcg:
...
def build_markdown(xlsx: str, output: str, run_id: str = "",
    ...
    lines = []
    lines.append("---")
    lines.append("tags: [tcg, scanner, myp, op, onepiece, arbitrage]")
    lines.append(f"date: {today}")
    lines.append(f"source: GH Actions run {run_id}" if run_id else "source: local scan")
    lines.append("---")
    lines.append("")
    lines.append(f"# MYP Scan ONE PIECE — {today}")
    lines.append("")
    ...
    lines.append(
        ...
        f"Referência: TCGplayer market via tcgcsv.com (cat 68 = One Piece "
        f"Card Game EN) · Câmbio US$1 = R${float(fx):.4f} · Ofertas: menor "
        f"EN NM ao vivo no MYP."
        if fx else
    ...
    lines.append("")
    if run_id:
        lines.append(f"**Artifact XLSX:** [`myp-op-consolidated-{run_id}`]"
    ...
    lines.append(f"*Gerado em {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} "
                 f"via `myp_op_summary.py` (single source: XLSX do scan OP). "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Gera o markdown de ENTREGA do scan MYP One Piece")
    parser.add_argument("xlsx", help="XLSX do myp_op_scanner.py")
